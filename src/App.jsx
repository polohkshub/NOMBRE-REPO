...
const CATEGORIES = ["casa", "personal", "kaguneta", "comida", "eventuales", "lolo"];
      ...
      <main className="main-content">
        <section className="card">
          ...
          <div className="form-grid">
            <label className="form-field">
              Monto
              <input type="number" placeholder="1500" value={amount} onChange={(e) => setAmount(e.target.value)} />
            </label>
            <label className="form-field">
              Descripción
              <input type="text" placeholder="supermercado..." value={desc} onChange={(e) => setDesc(e.target.value)} />
            </label>
            ...
            <button className="btn primary full-width" onClick={addExpense}>Guardar gasto</button>
          </div>
        </section>
        ...
        <section className="card">
          ...
          <div className="categories-grid">
            {CATEGORIES.map((c) => (
              <div key={c} className={`category-pill ${totalsByCat[c] > 0 ? 'active' : ''}`}>
                <div className="pill-title">{c.toUpperCase()}</div>
                <div className="pill-value">{formatARS(totalsByCat[c])}</div>
              </div>
            ))}
          </div>
          <button className="btn secondary full-width" onClick={exportResumen}>Exportar a Excel</button>
        </section>
        ...
        <section className="card">
                ...
                .map((it) => (
                  <div key={it.id} className="expense-item">
                    ...
                    <div className="expense-right">
                      ...
                      <button className="delete-btn" onClick={() => removeExpense(it.id)}>🗑</button>
                    </div>
                  </div>
                ))}
            </div>
          ...
          <div className="month-actions">
            <button className="btn primary" onClick={saveMonthSnapshot}>GUARDAR MES</button>
            <button className="btn secondary" onClick={loadMonthSnapshot}>CARGAR MES</button>
          </div>
          <div className="danger-zone">
            <button className="btn danger large" onClick={clearAll}>Borrar TODOS los gastos</button>
          </div>
